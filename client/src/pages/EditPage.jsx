...
const EditPage = () => {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        ...
        setError('فشل استرجاع بيانات الصفحة. قد تكون الصفحة غير موجودة.');
        setLoading(false);
      }
    };
    fetchPage();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'حدث خطأ أثناء تعديل وحفظ التغييرات.');
      setIsSaving(false);
    }
  };
...
`;
  ...
  return (
    <div className="p-4 lg:p-6 max-w-[1600px] mx-auto space-y-6 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-700/60 pb-5">
        <div>
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">
            ...
            <span>/</span>
            <span className="text-slate-400">تعديل الصفحة</span>
          </div>
          <h2 className="text-xl font-bold text-slate-100">تعديل صفحة الهبوط</h2>
          <p className="text-xs text-slate-400 mt-1">تحديث مسار الرابط، تعديل كود الصفحة، وإعداد السيو والتنسيقات الإضافية.</p>
        </div>
      ...
      {loading ? (
        <div className="bg-[#1e293b] border border-slate-700/60 rounded-2xl p-20 flex flex-col items-center justify-center shadow-lg">
          <div className="w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
          <p className="mt-4 text-[10px] font-bold text-slate-400 tracking-wide">جاري استرجاع بيانات الصفحة من الخادم...</p>
        </div>
      ) : error ? (
        <div className="bg-[#1e293b] border border-slate-700/60 rounded-2xl p-12 text-center space-y-4 shadow-lg">
          <div className="text-red-400 font-bold text-xs leading-relaxed">{error}</div>
          <Link
            to="/dashboard"
            className="inline-flex bg-slate-800 hover:bg-slate-750 border border-slate-700/60 px-4 py-2 rounded-xl text-[10px] font-bold text-blue-450 hover:text-blue-300 transition-colors"
          >
            العودة للوحة التحكم
          </Link>
        </div>
                ...
                <div className="space-y-5 animate-[fadeIn_0.2s_ease-out]">
                  ...
                  <div>
                    ...
                    <textarea
                      rows="6"
                      dir="ltr"
                      placeholder="// اكتب سكربتات تتبع مخصصة هنا&#10;console.log('Noor Tracker Active!');"
                      value={customJs}
                ...
                >
                  {isSaving ? (
                    <>
                      ...
                      <span>جاري حفظ وتحديث صفحتك...</span>
                    </>
                  ) : (
                    <>
